...
4. Run `npm run dev`; open `http://localhost:3000` in the Chrome profile with funded Preprod Lace. Connect, enter a strong local storage password in **Settings**, select **Save password**, expand **Local developer controls**, and click **Deploy new Preprod contract**. These controls are absent from production builds. Save the **finalized** 64-hex contract address and transaction ID. If finalization is uncertain, check the Preprod transaction before retrying.
