...
Checked **26 September 2026**. Legend: **✓ Complete**, **✗ Missing**, **⚠ Manual action required**. These are evidence statuses, not organizer approval.
...
The CSV has no transaction IDs or wallet-control attestations. SDK address validation does not independently link a public wallet to a private commitment or prove one person per wallet. This limits on-chain attribution; it does not reclassify the owner-confirmed testers as synthetic.
