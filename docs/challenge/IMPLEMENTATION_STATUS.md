...
| 2 | Responsive frontend, Lace connector code with opaque-key discovery, provider transaction path, clear-local-session flow, privacy explanation, and four Playwright flows passed. The owner confirmed Lace connected on Preprod and Kairos displayed an address. | Successful circuit transaction observed on Preprod; verified contract address and public demo URL; manual demo video; meaningful commit evidence. |
| 3 | Eleven contract, twenty-three unit, and four Playwright tests passed locally. CI workflow compiles, tests, typechecks, lints, and builds; local production build passed. `PROPOSAL.md` has the owner-requested placeholders. | Hosted passing CI run/badge and public demo; owner's proposal answers; organizer confirmation that Kairos fits the allowed category or explicit exception; meaningful commit evidence. |
